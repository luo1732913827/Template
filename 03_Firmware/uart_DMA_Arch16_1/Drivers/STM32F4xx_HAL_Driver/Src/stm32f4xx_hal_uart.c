      ...
      && ((cr1its & USART_SR_IDLE) != 0U))
  {
    ...
    if (HAL_IS_BIT_SET(huart->Instance->CR3, USART_CR3_DMAR))
    {
          ...
          && (nb_remaining_rx_data < huart->RxXferSize))
      {
        ...
        //HAL_UARTEx_RxEventCallback(huart, (huart->RxXferSize - huart->RxXferCount));//空闲中断
...
#else
    /*Call legacy weak Rx Event callback*/
    //HAL_UARTEx_RxEventCallback(huart, huart->RxXferSize); //全满中断
    dma_comp_irq_callback(huart->RxXferSize);
#endif /* USE_HAL_UART_REGISTER_CALLBACKS */
  }
  else
  {
...
#else
    /*Call legacy weak Rx Event callback*/
    //HAL_UARTEx_RxEventCallback(huart, huart->RxXferSize / 2U); //半满中断
